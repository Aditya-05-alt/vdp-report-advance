...
const LIVE_TABLE = "smart_hoot_inventory_live";
...
/**
 * Pipeline step 1 — Hoot live sync → smart_hoot_inventory_live.
 * Fetches all active dealers' hoot_url CSV, TRUNCATE + INSERT (no first_seen / last_seen).
 *
 * POST /functions/v1/smart-hoot-inv-live
 * Body (optional): { "platforms_only": false }
 */
serve(async (req) => {
  ...
  try {
    console.log(`Starting ${LIVE_TABLE} sync (smart-hoot-inv-live)...`);
    ...
    console.log(`Sync complete: ${insertedTotal} rows in ${LIVE_TABLE}`);
    ...
    console.error("smart-hoot-inv-live error:", message);
    ...
    return jsonResponse({ success: false, edge: "smart-hoot-inv-live", error: message }, 500);
  }
});
