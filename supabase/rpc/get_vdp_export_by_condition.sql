-- VDP export: date-wise rows per inv_url + condition (XLSX "By Condition" sheet).
-- URL from smart_final_data.inv_url. Deploy in Supabase SQL editor.

DROP FUNCTION IF EXISTS public.get_vdp_export_by_condition(
  text, date, date, text[], text[], text[], text[], integer[], text
);

CREATE OR REPLACE FUNCTION public.get_vdp_export_by_condition(
  p_client_id text,
  p_from date,
  p_to date,
...
AS $$
  WITH filtered AS (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END AS url,
      COALESCE(NULLIF(TRIM(inv_condition), ''), 'Unknown') AS condition,
  ...
  agg AS (
    SELECT report_date, url, condition, SUM(v)::bigint AS views
    FROM filtered
    WHERE url IS NOT NULL AND url <> '' AND v > 0
    GROUP BY report_date, url, condition
  )
  SELECT a.report_date, a.url, a.views, a.condition
  FROM agg a
  WHERE a.views > 0
  ORDER BY a.report_date DESC, a.views DESC, a.url, a.condition;
$$;

GRANT EXECUTE ON FUNCTION public.get_vdp_export_by_condition(
  text, date, date, text[], text[], text[], text[], integer[], text
