-- VDP export: date-wise rows per inv_url + make (XLSX "By Make" sheet).
-- URL from smart_final_data.inv_url. Deploy in Supabase SQL editor.

DROP FUNCTION IF EXISTS public.get_vdp_export_by_make(
  text, date, date, text[], text[], text[], text[], integer[], text
);

CREATE OR REPLACE FUNCTION public.get_vdp_export_by_make(
  p_client_id text,
  p_from date,
  p_to date,
...
AS $$
  WITH filtered AS (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END AS url,
      COALESCE(NULLIF(TRIM(inv_make), ''), 'Unknown') AS make,
  ...
  agg AS (
    SELECT report_date, url, make, SUM(v)::bigint AS views
    FROM filtered
    WHERE url IS NOT NULL AND url <> '' AND v > 0
    GROUP BY report_date, url, make
  )
  SELECT a.report_date, a.url, a.views, a.make
  FROM agg a
  WHERE a.views > 0
  ORDER BY a.report_date DESC, a.views DESC, a.url, a.make;
$$;

GRANT EXECUTE ON FUNCTION public.get_vdp_export_by_make(
  text, date, date, text[], text[], text[], text[], integer[], text
